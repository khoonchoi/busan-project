...
status
  ...
  - donated
*/
type Property struct { // world state 에 value 에 JSON으로 marshal 되어 저장될 구조체
	Sid   		string 	`json:"Sid"`
	Serial  	string 	`json:"Serial"`
	Renewer		string 	`json:"Renewer"`
	Doner		string  	`json:"Doner"`
...
-history(sid)
*/
func (s *SmartContract) Collect(ctx contractapi.TransactionContextInterface, sid string, serial string, donor string) error { 
	...
	return ctx.GetStub().PutState(sid, dAsBytes)  
	// value: JSON format
	// endorser peer 반환 -> 서명 -> APP -> orderer -> commiter 동기화
}

func (s *SmartContract) Query(ctx contractapi.TransactionContextInterface, rid string) (*Property, error) { 
	...
	// 정상적으로 조회가 된 경우
	diag := new(Property) // 객체화 JSON -> 구조체
	_ = json.Unmarshal(dAsBytes, diag) // call by REFERENCE

	return diag, nil
}

func (s *SmartContract) Renew(ctx contractapi.TransactionContextInterface, rid string, renewer string) error { // application - submitTransaction("ChangeCarOwner", "CAR10", "blockchain")
	
	diag, err := s.Query(ctx, rid)

	if err != nil {
		return err
	}
	// 검증 생략 -- 이전 소유자에서 전달되었는지 유효한지
	if diag.Status == "collected" {
		// 상태변경 
		diag.Status = "renewed"
		// (TO DO) 검증자 기록
